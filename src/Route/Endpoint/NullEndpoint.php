<?php

/*
 * This file is part of Polymorphine/Routing package.
 *
 * (c) Shudd3r <q3.shudder@gmail.com>
 *
 * This source file is subject to the MIT license that is bundled
 * with this source code in the file LICENSE.
 */

namespace Polymorphine\Routing\Route\Endpoint;

use Polymorphine\Routing\Route\Endpoint;
use Psr\Http\Message\ServerRequestInterface;
use Psr\Http\Message\ResponseInterface;


/**
 * Endpoint that can build Uri, but will not forward any request.
 * Useful as default URI generator for requests dispatched on same URI
 * (for example: resource URI for undefined set of http methods).
 */
class NullEndpoint extends Endpoint
{
    public function forward(ServerRequestInterface $request, ResponseInterface $prototype): ResponseInterface
    {
        return $this->execute($request, $prototype);
    }

    protected function execute(ServerRequestInterface $request, ResponseInterface $prototype): ResponseInterface
    {
        return $prototype;
    }
}
